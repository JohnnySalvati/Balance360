...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            'condicion_iva',
            postgresql.ENUM(
                'INSCRIPTO', 'EXENTO', 'FINAL', 'MONOTRIBUTO',
                name='condicioniva', create_type=False,
            ),
            nullable=False,
        ),
        sa.Column('tax_id', sa.String(length=11), nullable=False),
        sa.Column('iibb_rate', sa.Numeric(precision=5, scale=2), server_default='0', nullable=False),
    ...
    op.create_foreign_key(
        ...
        'invoices', 'fiscal_identities',
        ['fiscal_identity_id'], ['id'],
    )
...
def downgrade() -> None:
    ...
    op.add_column('entities', sa.Column('start_date', sa.DATE(), autoincrement=False, nullable=True))
    op.add_column(
        'entities',
        sa.Column(
            'condicion_iva',
    ...
    op.add_column('entities', sa.Column('address', sa.VARCHAR(length=200), autoincrement=False, nullable=True))
    op.add_column('entities', sa.Column('tax_id', sa.VARCHAR(length=13), autoincrement=False, nullable=True))
    op.add_column('entities', sa.Column('iibb_rate', sa.NUMERIC(precision=5, scale=2), server_default=sa.text("'0'::numeric"), autoincrement=False, nullable=False))
    op.add_column('entities', sa.Column('iibb', sa.VARCHAR(length=50), autoincrement=False, nullable=True))
