...
from balance360.services.import_rule import extract_amount, find_best_rule
...
def import_sheet(
    db: Session, rows: list[dict], account: Account, rules: list[ImportRule], source_file: str
):
    for row in rows:
        amount = row["amount"]
        ...
        stmt = (
            ...
            .values(**transaction_dict)
            .on_conflict_do_nothing(constraint="uq_transaction")
        )
        db.execute(stmt)
    db.commit()
...
if __name__ == "__main__":
    ...
    with SessionLocal() as db:
        ...
        for account in accounts:
            ...
            if cash_rows:
                export_rows(wb, cash_rows, "Efectivo")
            if visa_rows:
                export_rows(wb, visa_rows, "VISA Ciudad")

            print(
                f"Solapa {account.name}: {len(valid_rows)} filas validas, {len(skipped_rows)} filas saltadas, {len(cash_rows)} filas de efectivo creadas, {len(visa_rows)} filas VISA creadas"
            )
