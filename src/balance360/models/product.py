...
class Product(Base, TimestampMixin):
    ...
    margin: Mapped[Decimal] = mapped_column(Numeric(precision=10, scale=2))
    track_serial: Mapped[bool] = mapped_column(Boolean, default=False)

    serial_numbers: Mapped[list["SerialNumber"]] = relationship(back_populates="product")
    invoice_lines: Mapped[list["InvoiceLine"]] = relationship(back_populates="product")
