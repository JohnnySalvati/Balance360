...
Existe por `POST /api/tokens`, que es el primer endpoint de esta app que **acepta una
contraseña sin haber autenticado a nadie todavía**. Todo lo demás pide cookie o token: quien
se equivoca ahí ya tiene una credencial válida o no entra. Un endpoint que contesta "esta
contraseña no es" es un oráculo, y un oráculo sin límite se prueba entero.

**Es un piso, no el techo.** Un solo proceso cuenta lo suyo: con N workers el límite efectivo
es N veces el configurado. El techo de verdad va en el borde —Caddy, o un WAF—. Lo que sí
hace esta capa es acotar el daño sin depender de que el despliegue esté bien configurado, y
hacerlo con conocimiento que el borde no tiene: Caddy no sabe qué dirección de mail viene en
el body.
...
contador. Para lo que se defiende acá —que nadie pruebe contraseñas de a miles— esa ráfaga no
cambia nada, y un contador por clave es lo que hace que la memoria no crezca con el tráfico.
...
def reset_all() -> None:
    ...
    Es un registro y no una tupla escrita a mano en cada módulo: los limitadores son globales
    de módulo, así que uno que se olvide de agregarse a la lista le deja el contador cargado
    al test siguiente, y el que falla no es el que lo rompió sino el que quedó sexto.
    """
...
class RateLimiter:
    ...
    # tests puedan mover el tiempo en vez de dormir: la ventana real es de un cuarto de hora.
    # Se inyecta en vez de parchearse porque parchear `time.monotonic` lo cambia para todo el
    ...
    def _prune(self, now: float) -> None:
        ...
        Corre adentro del candado, en cada intento. Es O(claves vivas) y suena caro, pero las
        claves vivas son las que actuaron en la última ventana: si esa cantidad llegara a ser
        un problema de CPU, el problema real sería el ataque, no el barrido.
        """
        expired = [
            key
