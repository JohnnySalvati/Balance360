...
def sync_blue(db: Session) ->int:
    return sync_series(db=db, url="https://api.argentinadatos.com/v1/cotizaciones/dolares/blue", currency_code="USD", extractor="venta")
    
    
def sync_uva(db: Session) -> int:
    return sync_series(db=db, url="https://api.argentinadatos.com/v1/finanzas/indices/uva", currency_code="UVA", extractor="valor")
