...
def ars_rate_subquery(
    source_currency_id: UUID | None,
    source_date: date,
    target_currency_id: UUID | None,
    target_date: date,
):
    source = (
...
def conversion_factor(source_id, txn_date, target_currency, reference_date):
    if not target_currency:
        ...
        result = ars_rate_subquery(source_id, txn_date, None, txn_date) * ars_rate_subquery(
            target_currency.id, reference_date, target_currency.id, txn_date
        )
    else:
