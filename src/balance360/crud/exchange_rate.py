...
def get_by_id(db: Session, exchange_rate_id: uuid.UUID) -> ExchangeRate | None:
    exchange_rate = (
        db.execute(select(ExchangeRate).where(ExchangeRate.id == exchange_rate_id))
        .scalars()
        .first()
    )
...
def get_by_currency_and_date(
    db: Session, currency_id: uuid.UUID, date: date
) -> ExchangeRate | None:
    exchange_rate = (
        db.execute(
            select(ExchangeRate).where(
                ExchangeRate.currency_id == currency_id, ExchangeRate.date == date
            )
        )
        .scalars()
        .first()
