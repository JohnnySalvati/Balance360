...
def get_all(
    ...
    if q:
        stmt = stmt.where(SerialNumber.serial.ilike(f"%{q}%"))
...
def get_by_invoice_line_id(db: Session, invoice_line_id: uuid.UUID) -> list[SerialNumber]:
    ...
    if invoice_line.invoice.invoice_type == InvoiceType.purchase:
        serial_numbers = (
            db.execute(select(SerialNumber).where(SerialNumber.purchase_line_id == invoice_line_id))
            .scalars()
            .all()
        )
    else:
        serial_numbers = (
            db.execute(select(SerialNumber).where(SerialNumber.sale_line_id == invoice_line_id))
            .scalars()
            .all()
        )
