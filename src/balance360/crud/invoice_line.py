...
def get_by_invoice_product(db: Session, invoice_id: uuid.UUID, product_id: uuid.UUID) -> InvoiceLine|None:
    return db.execute(Select(InvoiceLine).where(InvoiceLine.invoice_id == invoice_id, InvoiceLine.product_id == product_id)).scalars().first()

def get_by_last_product_purchase(db: Session, product_id: uuid.UUID, entity_id: uuid.UUID) -> InvoiceLine|None:
    stmt = (
        Select(InvoiceLine)
        .where(Invoice.invoice_type == InvoiceType.purchase, Invoice.confirmed == True, Invoice.entity_id == entity_id)
