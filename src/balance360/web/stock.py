...
def serial_history(
    request: Request, serial_number: SerialNumber = Depends(get_serial_number_or_404)
):
    return templates.TemplateResponse(
        request=request,
        ...
        context={
            ...
            "client": serial_number.sale_line.invoice.contact.name
...
def stock(request: Request, db: Session = Depends(get_db), entity_id: str = Query(default="")):
