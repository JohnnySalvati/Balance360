...
def accounts_page(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        request=request, name="config/accounts/list.html", context={"accounts": accounts}
    )
...
@router.get("/rows")
def accounts_rows(request: Request, search: str = Query(default=""), db: Session = Depends(get_db)):

    accounts = account_crud.get_all(db, search)

    return templates.TemplateResponse(
        request=request, name="config/accounts/_rows.html", context={"accounts": accounts}
    )
...
def create_account(
    ...
    data = AccountCreate(
        name=name, type=AccountType(account_type), currency_id=uuid.UUID(currency_id)
    )
...
def account_edit_form(request: Request, account_id: uuid.UUID, db: Session = Depends(get_db)):
    return templates.TemplateResponse(
        request=request,
        name="config/accounts/_form_modal.html",
        context={
