...
def product_page(request: Request, db: Session = Depends(get_db)):
    return templates.TemplateResponse(
        request=request, name="products/list.html", context={"products": product_crud.get_all(db)}
    )
...
def product_rows(
    ...
    return templates.TemplateResponse(
        ...
        context={"products": product_crud.get_all(db, search)},
    )
...
def new_product_form(request: Request, db: Session = Depends(get_db)):
    return templates.TemplateResponse(
        ...
        context={"products": product_crud.get_all(db)},
    )
...
def create_product(
    ...
    data = ProductCreate(
        name=name, margin=Decimal(margin) if margin else Decimal(0), track_serial=track_serial
    )
...
def product_edit_form(request: Request, product_id: uuid.UUID, db: Session = Depends(get_db)):
    return templates.TemplateResponse(
        request=request,
        name="products/_form_modal.html",
        context={"product": product_crud.get_by_id(db, product_id)},
    )
