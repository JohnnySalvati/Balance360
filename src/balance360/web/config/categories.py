...
def categories_page(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        request=request, name="config/categories/list.html", context={"categories": categories}
    )
...
@router.get("/rows")
def categories_rows(
    request: Request, search: str = Query(default=""), db: Session = Depends(get_db)
):
    categories = category_crud.get_all(db, search)

    return templates.TemplateResponse(
        request=request, name="config/categories/_rows.html", context={"categories": categories}
    )
...
def create_category(
    ...
    data = CategoryCreate(
        name=name, parent_id=uuid.UUID(parent_id) if parent_id else None, description=description
    )
